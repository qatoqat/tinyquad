//! Tests for the desktop file-loading path of [`tinyquad::fs`].
//!
...
fn load(path: &str) -> tinyquad::fs::Response {
    let (tx, rx) = mpsc::channel();
    tinyquad::fs::load_file(path, move |response| {
...
fn loads_existing_file() {
    ...
    path.push("tinyquad_fs_test_existing.txt");
    std::fs::write(&path, b"tinyquad file contents").unwrap();

    let response = load(path.to_str().unwrap());

    assert_eq!(response.unwrap(), b"tinyquad file contents");
...
fn reports_missing_file_as_io_error() {
    let response = load("/definitely/not/a/real/path/tinyquad_test");

    let err = response.expect_err("missing file must produce an error");
    match err {
        tinyquad::fs::Error::IOError(_) => {}
...
#[test]
fn fs_error_display() {
    let err = tinyquad::fs::Error::DownloadFailed;
    assert_eq!(err.to_string(), "Download failed");

    let err = tinyquad::fs::Error::AndroidAssetLoadingError;
    assert_eq!(err.to_string(), "[android] Failed to load asset");

    let io: tinyquad::fs::Error = std::io::Error::new(std::io::ErrorKind::NotFound, "nope").into();
