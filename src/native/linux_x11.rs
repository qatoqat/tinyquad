...
use crate::{
    ...
    event::EventHandler,
    native::{NativeDisplayData, Request, egl, gl, module},
};
        ...
        unsafe {
            match event.type_0 {
                ...
                35 if Some(event.xcookie.extension) == self.libxi.xi_extension_opcode
                    && event.xcookie.evtype == xi_input::XI_RawMotion =>
                {
            ...
            // send ClientMessage to the window with request to change property to fullscreen
            {
                ...
                let mut ev = XClientMessageEvent {
                    ...
                    data: ClientMessageData {
                        l: std::mem::transmute::<[isize; 5], [libc::c_long; 5]>(data),
                    },
                };
...
// Returning the display in `Err` lets `start` fall back to the wayland backend;
// moving `X11Display` into a `Box` just to shrink the `Err` variant is not worth it.
