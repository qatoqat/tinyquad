// Imagine crate A depending on tinyquad.
// On android A is being compiled to .so and this .so is being loaded
// by "System.loadLibrary("")" java call.
// extern "C" functions from tinyquad are right there, in the .so. But somehow
...
//
// But we really need to be able to make tinyquad's functions visible to java.
//
...
// and they just forward the call to the real implementation inside tinyquad
// Note that because it is being injected - we might not have neither tinyquad
