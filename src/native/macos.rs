...
use {
    ...
    std::{
        collections::HashMap,
        os::raw::c_void,
        sync::{Condvar, Mutex, mpsc::Receiver},
...
pub fn define_cocoa_window_delegate() -> *const Class {
    extern "C" fn window_should_close(this: &Object, _: Sel, _: ObjcId) -> BOOL {
        ...
        if handle_quit(payload) { YES } else { NO }
    }
    ...
    unsafe {
        ...
        extern "C" fn dragging_entered(
            ...
            unsafe {
                ...
                let matched_type: ObjcId =
                    msg_send![pasteboard, availableTypeFromArray: file_types];
                if matched_type != nil {
...
// The `&mut MacosDisplay` comes from a display pointer stored in the view's ivar,
    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    unsafe {
        ...
        let view = match conf.platform.apple_gfx_api {
            AppleGfxApi::OpenGl => {
                create_opengl_view(&mut display, conf.sample_count, conf.high_dpi)
            }
        ...
        let timer = match conf.platform.apple_gfx_api {
            AppleGfxApi::OpenGl => {
                msg_send_![class!(NSTimer), timerWithTimeInterval:0.016 // ~60FPS
                                                           target:view
